...
export async function POST(req: Request, { params }: { params: Promise<{ username: string }> }) {
  try {
    ...
    // ========== ADD TRAFFIC (بدون تغییر پروفایل) ==========
    if (action === 'add_traffic') {
      if (!body.bytes) {
        return NextResponse.json({ error: 'Missing bytes' }, { status: 400 });
      }
      await query(`
        UPDATE dashboard_users
        SET bonusBytes = COALESCE(bonusBytes, 0) + ?
        WHERE username = ?
      `, [body.bytes, username]);
    ...
    const userResult: any = await query(
      `SELECT firstName, lastName, \`group\`, expiration FROM dashboard_users WHERE username = ? LIMIT 1`,
      [username]
    );
    if (userResult && userResult.length > 0) {
      ...
      if (u.group) {
        const profileResult: any = await query(
          `SELECT price, validityDays, totalTraffic FROM dashboard_profiles WHERE name = ? LIMIT 1`,
          [u.group]
        );
